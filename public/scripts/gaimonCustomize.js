...
function createGlassesMenu() {
  ...
  glassesMenu.setAttribute("name", "glasses-menu");
...
async function changeMood() {
  gaimon_mood_checkbox = await document.getElementById("mood-checkbox");
  gaimon_mood_checkbox.checked = !gaimon_mood_checkbox.checked;
  console.log("Checkbox mood state: ", gaimon_mood_checkbox.checked);

  gaimon_image = await document.getElementById("gaimon-image");
  mood_button = await document.getElementById("mood-button");
  if (
    gaimon_image.getAttribute("src") == "../public/images/gaimon_stressed.webp"
  ) {
...
async function showHideGlasses(show) {
  ...
  if (show) glasses.style.visibility = "visible";
}
